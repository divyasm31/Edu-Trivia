...
function SideBarDashboard() {
    ...
    function logOut(){
      ...
      const handleOptionSelect = (eventKey) => {
        ...
        navigate(`/user-profile/${currentUser.username}/${eventKey}`, { state: { eventKey } });
      };
      const toggleSidebar = () => {
    ...
    <div className='bg-transparent'>
      ...
      <div className={`sidebar ${isSidebarVisible ? 'd-block' :'d-none'}`}>
        <ul className='list-unstyled p-2 mt-5 me-5 ms-2 '>
          ...
          <li className='mb-4'>
            <NavLink to={`/stats/${currentUser.username}`} className='lead header-para ps-1' style={{ textDecoration: "none", color: "lemonchiffon"  }}>
                Leaderboard
            </NavLink>
          </li>
          <li>
